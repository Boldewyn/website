...
class BootstrapTestCase(unittest.TestCase):
    ...
    def test_existing_nonempty_target(self):
        ...
        self.assertRaises(ValueError, bootstrap,
                          os.path.join(self.tmpdir, "nonempty_target"), {})
    ...
    def test_usual(self):
        ...
        self.assertTrue(os.path.isfile(os.path.join(self.tmpdir,
                        "usual/_config.py")), "config file not added")
    ...
    def test_in_docroot(self):
        ...
        self.assertTrue(os.path.isfile(os.path.join(self.tmpdir,
                        "in_docroot/robots.txt")), "robots.txt not added")
        self.assertTrue(os.path.isfile(os.path.join(self.tmpdir,
                        "in_docroot/humans.txt")), "humans.txt not added")
    ...
    def test_below_docroot(self):
        ...
        self.assertFalse(os.path.isfile(os.path.join(self.tmpdir,
                         "below_docroot/robots.txt")), "robots.txt not added")
        self.assertFalse(os.path.isfile(os.path.join(self.tmpdir,
                         "below_docroot/humans.txt")), "humans.txt not added")
...
class BuildTestCase(unittest.TestCase):
    ...
    def test_archive(self):
        ...
        build({"BUILD_TARGET": os.path.join(self.tmpdir, "site")})
        self.assertTrue(os.path.isdir(os.path.join(self.tmpdir,
                        "site/archive")), "archive directory not built")
        self.assertTrue(os.path.isfile(os.path.join(self.tmpdir,
                        "site/archive/%s/index.html" % datetime.now().year)),
                        "archive/year directory not built")
        self.assertTrue(os.path.isfile(os.path.join(self.tmpdir,
                        "site/archive/%s/%02d/index.html" % (datetime.now().year,
                        datetime.now().month))), "archive/year/month not built")
